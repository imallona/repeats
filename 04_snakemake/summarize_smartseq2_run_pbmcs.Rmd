...
```{r}
if (params$repeats_only_unique != "None") {
    
    common_cells <- intersect(colnames(d$genes_star), colnames(d$repeats_only_unique))
    ## length(common_cells)
    d$both_unique <- CreateSeuratObject(
        rbind(as.matrix(GetAssayData(d$genes_star, slot = 'counts'))[,common_cells],
              as.matrix(GetAssayData(d$repeats_only_unique, slot = 'counts'))[,common_cells]),
        assay = "RNA",
        min.cells = 0,
        min.features = 0,
        names.field = 1,
...
```{r}
if (params$repeats_only_multi != "None") {
    
    common_cells <- intersect(colnames(d$genes_star), colnames(d$repeats_only_multi))
    ## length(common_cells)
    d$both_multi <- CreateSeuratObject(
        rbind(as.matrix(GetAssayData(d$genes_star, slot = 'counts'))[,common_cells],
              as.matrix(GetAssayData(d$repeats_only_multi, slot = 'counts'))[,common_cells]),
        assay = "RNA",
        min.cells = 0,
        min.features = 0,
        names.field = 1,
...
for (item in grep('repeats|both', names(d), value = TRUE)) {
    rownames(d[[item]]@meta.data) <- d[[item]]@meta.data$cell
