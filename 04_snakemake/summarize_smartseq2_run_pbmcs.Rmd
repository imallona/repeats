...
for (ct in unique(pos_markers2_tmp$cell_type)) {
    ...
    mapping <- data.frame(
        id = rownames(d$genes),
        symbol = sapply(strsplit(rownames(d$genes), '-'), function(x) return (x[2])),
        ensg = sapply(strsplit(rownames(d$genes), '-'), function(x) return (x[1])),
...
## res <- 'RNA_snn_res.0.2'
common_cells <- intersect(colnames(d$genes_star), colnames(d$repeats_star))
