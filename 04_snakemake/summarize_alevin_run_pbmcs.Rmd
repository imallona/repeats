...
d$genes_alevin@meta.data <- merge(d$genes_alevin@meta.data, labelling, by = 'cell')
...
```{r problemrendering, cache = FALSE}
...
@todo delete me, coef_var is pointless

```{r fig.height = 6, fig.width = 16, results = 'asis', eval = FALSE}
## selected <- c('AluY', 'AluSx', 'MER49', 'MIR', 'L2a', 'L2b', 'L1PA11')
for (slot in c('repeats_alevin')) {
    cat("## ", slot, "\n\n")
    ...
    for (flavour2 in setdiff(names(d), flavour1)) {
        ...
        for (i in 1:5) {
            for (j in 1:5) {
                ...
                ## if (i == 1 & j == 1) {
                    legend('top',
                           legend = levels(d[[flavour1]]@meta.data[common, 'scina']),
