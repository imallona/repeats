...
runmeta <- sapply(GSMList(gse), function(x) return( c(Meta(x)$relation, Meta(x)$title)))
...
cellmeta[cellmeta$tech == 'CEL-Seq2',] # so the barcode is prepended with the run number
...
libsize.drop <- isOutlier(d$genes_alevin$nCount_RNA, nmads = 2, type = "both", log = TRUE)
feature.drop <- isOutlier(d$genes_alevin$nFeature_RNA, nmads = 2, type = "both", log = TRUE)
mito.drop <- isOutlier(d$genes_alevin$percent.mt, nmads = 2, type = "higher")
