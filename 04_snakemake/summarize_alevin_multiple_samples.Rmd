...
for (run in genes_fns) {
    counts[[run]] <- tximeta::tximeta(coldata = data.frame(
                                names = "genes",
                                files = file.path(params$genes_alevin, run),
...
process_featurecounts_grouped_by_cb_umi <- function(fn, annotate_genes = FALSE) {
     ## Featurecounts-based repeats
    counts <- data.table::fread(fn, sep = '\t', header = FALSE)
    ## cb first, umi second
    ...
    if (annotate_genes) {
        ...
        tmp <- merge(data.frame(ensembl_gene_id_version = rownames(wide)),
                     by = 'ensembl_gene_id_version', feature_info, all.x = TRUE)

        rownames(tmp) <- tmp$ensembl_gene_id_version
        tmp <- tmp[rownames(wide),]

        stopifnot(rownames(wide) == tmp$ensembl_gene_id_version)

        rownames(wide) <- sprintf('%s_%s', tmp$ensembl_gene_id_version, tmp$external_gene_name)
...
```{r}
# test
fn1 <-  file.path('/home/imallona/repeats_sc/runs',
                  'ding_seqwell/bowtie_repeatome/multimappers/SRR9169418.bam.featureCounts.gz')

(process_featurecounts_grouped_by_cb_umi(fn1, annotate_genes = FALSE))[1:10, 1:3]

fn2 <- '/home/imallona/repeats_sc/runs/ding_seqwell/star_transcriptome/genes/SRR9169214_Aligned.sortedByCoord.out.bam.featureCounts.gz'

process_featurecounts_grouped_by_cb_umi(fn2, annotate_genes = TRUE)[1:10, 1:3]
...
for (item in grep('repeats|genes', grep('alevin', names(params), invert = TRUE))) {
