...
if (params$id == 'CEL-Seq2') {
    ...
    dict$rep <- sapply(strsplit(as.character(dict$V3), '_'),
...
get_count_table_grouped_by_dings_run <- function(alevin_fns) {
    ...
    for (run in ding$run) {
        ...
        for (fn in ding[ding$run == run, 'fns']) {
            ...
            counts[[run]][[srr]] <- tximeta::tximeta(coldata = data.frame(
                                                         names = "genes",
                                                         files = file.path(params$genes_alevin, fn),
                                                         stringsAsFactors = FALSE
                                                     ), type = "alevin")
...
counts <- get_count_table_grouped_by_dings_run(genes_fns) 
...
counts <- get_count_table_grouped_by_dings_run(repeats_fns)
