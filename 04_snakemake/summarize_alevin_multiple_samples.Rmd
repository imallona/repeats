...
if (params$id == 'CEL-Seq2') {
    ...
    cellmeta$run <- sapply(strsplit(as.character(cellmeta$id), '-'), function(x) return(x[1]))
}
tail(cellmeta)
...
get_count_table_grouped_by_dings_run <- function(alevin_fns, dict, cellmeta,
    ...
    for (run in ding$run) {
        ...
        cbs <- cellmeta[cellmeta$run == run, 'barcode']
...
for (item in grep('repeats|genes',
    ...
    for (run in unique(as.character(ding$run))) {
        ...
        ## rather than merge them, sum them
        genenames <-  unique(unlist(sapply(counts[[run]], rownames)))
