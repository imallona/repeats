...
For each sample, each flavour, and each celltype, get the top 100 markers. Then, look for overlaps.
...
for (rds in params) {    
    ...
    for (fn in fns) {
        ...
        ## get only the top 50 per celltype
...
for (celltype in unique(markers$cluster)) {
    ...
    for (dataset in unique(markers[markers$cluster == celltype, 'dataset'])) {
        ...
        ul[[paste(celltype, dataset, sep = "_")]] <- markers[ markers$cluster == celltype &
                                                              markers$dataset == dataset, 'gene']
    }
    ## str(ul)
    print(upset(fromList(ul), nintersects = NA))

    ul <- list()
    for (flavour in unique(markers[markers$cluster == celltype, 'flavour'])) {

        ul[[paste(celltype, flavour, sep = "_")]] <- markers[ markers$cluster == celltype &
                                                              markers$flavour == flavour, 'gene']
    }
    ## str(ul)
    print(upset(fromList(ul), nintersects = NA))
