...
for (ct in names(annot)) {
    ...
    annot[[ct]]$mapper <- 'NA'
    annot[[ct]][grepl('cellranger|star|featurecounts|only', annot[[ct]]$flavour), 'mapper'] <- 'star'
...
ham_d <- dist(t(fc[[item]]), method = 'manhattan')
...
full <- tree +
    ...
    geom_th_addvalue(name = "hm0", color = "white", size = 4) +
    ...
    geom_th_text(name = "hmtech", side = "top", nudge_y = 0.5,
                 angle = 90, size = 3) +
    ...
    geom_th_title(name = "hm0", side = "top",
                  label = "Num. discordant markers", nudge_y = -0.8) +
...
for (item in paste0('p', 1:8)) {

    leg[[item]] <- get_legend(get(item))
}


leg12 <- plot_grid(leg[[1]], leg[[2]],  ncol = 1)
leg35 <- plot_grid(leg[[3]], leg[[4]], leg[[5]], ncol = 1)
leg68 <- plot_grid(leg[[6]], leg[[7]], leg[[8]],  ncol = 1)

plot_grid(full + theme(legend.position = "none"), leg12, leg35, leg68,
          ncol = 4,
...
Tests on legends based on https://stackoverflow.com/questions/52060601/ggplot-multiple-legends-arrangement
...
l1 <- get_legend(one)
l2 <- get_legend(two)
leg12 <- plot_grid( l1, l2, ncol = 2)


plot_grid(full + theme(legend.position = "none"), leg12, nrow = )
```
