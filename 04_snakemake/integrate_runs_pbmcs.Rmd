...
```{r}
print(sort(unique(d$zheng_truth$genes_cellranger@meta.data$truth)))
print(sort(unique(d$zheng_truth$repeats_cellranger@meta.data$truth)))

print(sort(unique(zheng_pal$truth)))
saveRDS(d$zheng_truth, file = '~/tmp/debug_zheng.rds')
...
other_markers <- unique(markers_both[grepl('repeats', markers_both$flavour) & !markers_both$cluster == 'CD14+_monocyte',])

length(unique(monocyte_markers$gene))
