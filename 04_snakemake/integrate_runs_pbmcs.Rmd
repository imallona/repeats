...
```{r, results = 'asis'}
dataset <- 'zheng_truth'
for (flavour in names(d$zheng_truth)) {
    cat('## ', flavour, ' {.tabset .tabset-fade .tabset-pills} \n\n')

    ## print(head(d[[dataset]][[flavour]]@meta.data$truth))

    ## d[[dataset]][[flavour]]@meta.data$truth <- as.character(d[[dataset]][[flavour]]@meta.data$orig.ident)
...
#  max F1s *regardless of the resolution*
...
```{r, fig.width = 6, fig.height=7, results = 'asis'}
...
for (flavour in unique(sapply(avgs, names))) {
    for (item in names(d[[flavour]])) {

        
        common <- intersect(rownames(avgs[[1]][[flavour]][1]),
                            rownames(GetAssayData(d[[flavour]][[item]])))
        ## plot(AverageExpression(d[[flavour]][[item]]),
        ##      avgs[[1]][[flavour]][1])
        ## }

        ## better normalize this
        plot(AverageExpression(object = d$pbmc8k$repeats_cellranger)$RNA[common,'CD14+_monocyte'],
             avgs[[1]][[flavour]][1][common,])
}
