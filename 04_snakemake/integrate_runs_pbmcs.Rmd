...
for (dataset in names(d)) {
    ...
    d[[dataset]]$repeats_multi_unique <- d[[dataset]]$control_repeats_multi_unique
}
```
        ...
        for (flavour in levels(cobra$flavour)) {
            for (cell in levels(cobra$cell)) {
                maxF1[[paste(run, flavour, cell)]] <- c(
                    run, flavour, cell, max(cobra[cobra$flavour == flavour &
                                                  cobra$cell == cell &
                                                  cobra$id == run, 'F1']))
            }
        }
...
## table(maxF1$flavour, useNA = 'always')
## head(maxF1)
