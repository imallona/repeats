...
For each sample, each flavour, and each celltype, get the top 5 markers. Then, look for overlaps.
...
for (rds in params) {
    ...
    ## aris[[sapply(strsplit(dirname(rds), .Platform$file.sep),
    ##       function(x) return(x[6]))]] <- list()
    fns <- list.files(dirname(rds), pattern = 'markers.*rds')
    for (fn in fns) {
        curr_nest <- readRDS(file.path(dirname(rds), fn))
        for (item in names(curr_nest)) {
        ...
        ## get only the top 5 per celltype
...
listInput <- list(
    ...
    two = c(1, 2, 4, 5, 10), three = c(1, 5, 6, 7, 8, 9, 10, 12, 13))
