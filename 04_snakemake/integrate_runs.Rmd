...
```{r, fig.height = 4, fig.width = 8, results="asis", message = FALSE} 
for (run in names(d)) {
    cat("## ", run, " {.tabset}\n\n")
    ## print(run)
...
for (run in names(d)) {
    ...
    for (item in names(d[[run]])) {
        cat("### ", run, item, " {.tabset}\n\n")
        set.seed(4)
        idx_feat <- sample(rownames(GetAssayData(d[[run]][[item]])), NCELLS, replace = FALSE)
        idx_cell <- sample(colnames(GetAssayData(d[[run]][[item]])), NFEATURES, replace = FALSE)

        
        
        print(pheatmap(GetAssayData(object = subset(d[[run]][[item]], features = idx_feat,
                                                    cells = idx_cell)),
                       color = viridis(100),
                       cluster_rows = TRUE,
                       cluster_cols = TRUE,
