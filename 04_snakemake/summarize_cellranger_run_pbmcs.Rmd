...
```{r problemrendering, results = 'asis', df_table = 'default'}
...
```{r, fig.width =14, fig.height = 7, results = 'asis'}
for (item in names(d)) {
    cat("## ", item, "\n\n")
    ## d[[item]]@meta.data$scina <- as.factor(d[[item]]@meta.data$scina)
    Idents(d[[item]]) <- 'scina'
    p1 <- DimPlot(d[[item]], reduction = "umap", group.by = 'ident')
    p2 <- DimPlot(d[[item]], reduction = "tsne", group.by = 'ident')
...
```{r fig.height = 6, fig.width = 24, results = 'asis'}
## selected <- c('AluY', 'AluSx', 'MER49', 'MIR', 'L2a', 'L2b', 'L1PA11')
for (slot in c('repeats', 'featurecounts')) {
    cat("### ", slot, "\n\n")
