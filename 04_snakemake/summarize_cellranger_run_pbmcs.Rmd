...
summaries_cg <- list(genes_cellranger = file.path(
                  ...
                  repeats_cellranger = file.path(
                      ...
                      'outs', 'metrics_summary.csv'))
summaries_fc <- list(
    repeats_no_overlap = file.path(base, params$identifier, 'count_repeats_on_cellranger_standard_not_overlapping_genes', paste0(params$identifier, '_transcriptome_repeats_not_genes.counts.summary.png_barplot.csv')),
    repeats_featurecounts = file.path(base, params$identifier, 'count_repeats_on_cellranger_standard',
...
hungarian_weber <- function(clus_algorithm, clus_truth) {
...
hungarian weber should get 'truth' names
...
for (genes_res in seq(0.2, 2, 0.2)) {
    ...
    length(as.factor(d$genes_cellranger@meta.data[known, genes_res]))
    length(as.factor(d$genes_cellranger@meta.data[known, 'scina']))
    hung[[i]] <- hungarian_weber(as.numeric(d$genes_cellranger@meta.data[known, genes_res]),
                                 as.numeric(d$genes_cellranger@meta.data[known, 'scina']))

    ## c(sprintf('genes_%s (n=%s)', genes_res,
    ##                        length(unique(d$genes_cellranger@meta.data[known, genes_res]))),
    ##                sprintf('scina_%s (n=%s)', 'scina',
    ##                        length(unique(d$genes_cellranger@meta.data[known, 'scina']))),
    ##                adjustedRandIndex(d$genes_cellranger@meta.data[known, genes_res],
...
## hung <- do.call(rbind.data.frame, hung)
## colnames(hung) <- c('genes_res', 'scina', 'ari')
## hung$ari <- as.numeric(as.character(hung$ari))
str(hung)
```
...
clus_algorithm <- d$genes_cellranger@meta.data[known, genes_res]
str(clus_algorithm)

clus_truth <- d$genes_cellranger@meta.data[known, 'scina']
as.character(clus_truth)
...
hungarian_weber_with_labels <- function(clus_algorithm, clus_truth) {    
    ...
    ## add back proper naming
    ## names(pr)
    ## names(clus)
    ## class(labels_matched)
...
for (genes_res in seq(0.2, 2, 0.2)) {
    ...
    length(as.factor(d$genes_cellranger@meta.data[known, genes_res]))
    length(as.factor(d$genes_cellranger@meta.data[known, 'scina']))
    hung[[genes_res]] <- hungarian_weber_with_labels(d$genes_cellranger@meta.data[known, genes_res],
                                 d$genes_cellranger@meta.data[known, 'scina'])
