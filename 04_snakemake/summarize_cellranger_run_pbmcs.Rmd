...
for (item in c('repeats', 'featurecounts', 'control_repeats_only')) {
    rownames(d[[item]]@meta.data) <- d[[item]]@meta.data$cell
...
for (slot in c('repeats', 'featurecounts', 'control_repeats_only')) {
    cat("## ", slot, "\n\n")
...
## res <- 'RNA_snn_res.0.2'
common_cells <- intersect(intersect(colnames(d$genes), colnames(d$repeats)),
                                    intersect(colnames(d$featurecounts), colnames(d$control_repeats_only)))
...
for (cell in intersect( colnames(GetAssay(d$control_repeats_only, 'RNA')),
                       colnames(GetAssay(d$featurecounts, 'RNA')))) {
    reps <- intersect(rownames(d$control_repeats_only), rownames(d$featurecounts))
...
for (re in intersect( rownames(GetAssay(d$control_repeats_only, 'RNA')),
    ...
    cells <-  intersect(colnames(d$control_repeats_only), colnames(d$featurecounts))
...
for (control_res in seq(0.2, 2, 0.2)) {
    ...
    for (genes_res in seq(0.2, 2, 0.2)) {
        ...
        cells <- intersect(rownames(d$control_repeats_only@meta.data), rownames(d$genes@meta.data))
        ...
        aris[[i]] <- c(sprintf('control_fc_%s (n=%s)', control_res,
                       ...
                       adjustedRandIndex(d$control_repeats_only@meta.data[cells,control_res],
                                         d$genes@meta.data[cells,genes_res]))
        i <- i +1
    }    
}
