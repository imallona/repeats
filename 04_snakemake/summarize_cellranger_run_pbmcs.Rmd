...
Mixed (genes + repeats not overlapping genes) object generation


```{r}

common_cells <- intersect(colnames(d$genes_cellranger), colnames(d$control_repeats_only))

d$both <- CreateSeuratObject(
    rbind(GetAssayData(d$genes_cellranger, slot = 'counts')[,common_cells],
          GetAssayData(d$control_repeats_only, slot = 'counts')[,common_cells]),
    assay = "RNA",
    min.cells = 0,
    min.features = 0,
    names.field = 1,
...
for (item in c('both', grep('repeats', names(d), value = TRUE))) {
    rownames(d[[item]]@meta.data) <- d[[item]]@meta.data$cell
