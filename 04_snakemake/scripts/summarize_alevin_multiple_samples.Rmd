...
params <- list(identifier = "CEL-Seq2",
               seurat_output = "/home/imallona/tmp/test.rds",
               repeats_alevin = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/repeats',
               genes_alevin = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/genes',
               ...
               config_fn = '/home/imallona/repeats_sc/data/ding_celseq2/celseq2_ding.conf')

params <- list(identifier = "Seq-Well",
               seurat_output = "/home/imallona/tmp/test.rds",
               repeats_alevin = '/home/imallona/repeats_sc/runs/ding_seqwell/alevin/repeats',
               genes_alevin = '/home/imallona/repeats_sc/runs/ding_seqwell/alevin/genes',
               ...
               regress_genes_nCount = FALSE,
               regress_genes_nFeatures = FALSE,
               config_fn = '/home/imallona/repeats_sc/data/ding_seqwell/seqwell_ding.conf')


params <- list(identifier = "Drop-seq",
               seurat_output = "/home/imallona/tmp/test.rds",
               repeats_alevin = '/home/imallona/repeats_sc/runs/ding_dropseq/alevin/repeats',
               genes_alevin = '/home/imallona/repeats_sc/runs/ding_dropseq/alevin/genes',
               repeats_star_unique = "/home/imallona/repeats_sc/runs/ding_dropseq/star_transcriptome/repeats/unique_reads", 
               repeats_star_multi = "/home/imallona/repeats_sc/runs/ding_dropseq/star_transcriptome/repeats/multimappers", 
               repeats_bowtie_unique = "/home/imallona/repeats_sc/runs/ding_dropseq/bowtie_repeatome/unique_reads", 
               repeats_bowtie_multi = "/home/imallona/repeats_sc/runs/ding_dropseq/bowtie_repeatome/multimappers", 
               repeats_only_unique = "/home/imallona/repeats_sc/runs/ding_dropseq/star_transcriptome/repeats_no_overlap/unique_reads",
               repeats_only_multi = "/home/imallona/repeats_sc/runs/ding_dropseq/star_transcriptome/repeats_no_overlap/multimappers",
               genes_featurecounts = "/home/imallona/repeats_sc/runs/ding_dropseq/star_transcriptome/genes",
               regress_genes_nCount = FALSE,
               regress_genes_nFeatures = FALSE,
               config_fn = '/home/imallona/repeats_sc/data/ding_dropseq/dropseq_ding.conf')
...
## Filtering barcodes used by Ding et al

```{r, eval = TRUE, message = FALSE}

## Data from GEO, e.g. srx and runname (to get the biological replicate)

gseid <- 'GSE132044'

gse <- getGEO(gseid, GSEMatrix=FALSE)

runmeta <- as.data.frame(t(sapply(GSMList(gse),
                                  function(x) return( c(Meta(x)$relation, Meta(x)$title)))))

runmeta$srx <- sapply(strsplit(as.character(runmeta$V2), split = "="), function(x) return(x[2]))
head(runmeta$srx)


##  cell metadata from Ding et al (to get the used barcodes)

supp <- getGEOSuppFiles(gseid, makeDirectory = TRUE, baseDir = getwd(),
  fetch_files = TRUE, filter_regex = 'GSE132044_pbmc_hg38_cell.tsv.gz')

cellmeta <- read.table(rownames(supp), sep = '.', col.names = c('replicate', 'tech', 'id'))
cellmeta <- cellmeta[cellmeta$tech == params$id,]

cellmeta$barcode <- gsub('R?[0-9]+-', '', cellmeta$id)

if (params$id == 'CEL-Seq2') {
    cellmeta$run<- paste(cellmeta$tech, gsub('-[A-Z]+', '', cellmeta$id), sep = "-")
} else if (params$id == "Seq-Well"){
    ## cellmeta$run <-  paste0('Seq-Well-', gsub('-[A-Z]+', '', cellmeta$id))
    cellmeta$run <-  paste0('Seq-Well-',
                            sapply(strsplit(as.character(cellmeta$id), split ='-'),
                                   function(x) return(x[1])))
} else if (params$id == 'Drop-seq') {
    cellmeta$run <- sprintf('%s-%s', params$id, cellmeta$replicate)
}
tail(cellmeta)

## our srr downloaded files (to be annotated for biological replicate/used barcodes)

## conf <- list.files(dirname(params$genes_alevin, '..', '*conf'))

conf <- read.csv(params$config_fn, header = FALSE)
head(conf)
...
generate_dict <- function(conf, runmeta) {
    dict <- merge(conf[,c('V13', 'V1')], runmeta[,c('srx', 'V3' )], by.x = 'V13', by.y ='srx')

    dict$pbmc <- sapply(strsplit(as.character(dict$V3), split ='\\.'), function(x) return (x[1]))

    if (params$id == 'CEL-Seq2') {
        dict$rep <- substr(as.character(dict$V3),
                           nchar(as.character(dict$V3)),
                           nchar(as.character(dict$V3)))
    } else if (params$id == "Seq-Well") {
        ## this weird thing is because cellmeta and  the GEO runs are unconsistent
        dict$rep <- gsub('_NA', '', sapply(strsplit(as.character(dict$V3), '_'),
                                           function(x) return(paste0(x[2], '_', x[3]))))

        dict$rep <- gsub('V1', '', dict$rep)
    } else {
        dict$rep <- dict$V3
    }

    if (params$id %in% c('Seq-Well', 'CEL-Seq2')) {
        dict$run <- paste(params$identifier, dict$rep, sep = '-')
    } else if (params$id == 'Drop-seq') {
        dict$run <- sprintf('%s-%s', params$id, dict$pbmc)
    }

    if(params$id == 'Drop-seq') {
        colnames(dict) <- c('srx', 'srr', 'full', 'pbmc', 'rep', 'run')
    } else {
        colnames(dict) <- c('srx', 'srr', 'full', 'rep', 'pbmc', 'run')
    }
...
```{r}
(dict <- generate_dict(conf = conf, runmeta = runmeta))

```

Now, subset the genes/repeats alevin slots to contain only Ding's approved barcodes

```{r}

get_ding_approved_barcodes <- function(cellmeta, dict, srr) {
    dict <- dict[dict$srr == srr,]
    tmp <- merge(cellmeta, dict, by = 'run')
    ## if (append_srr)
    ##     return(paste(tmp$srr, tmp$barcode, sep = "_"))
    ## else
    ##     return(as.character(tmp$barcode))
    if (params$id == 'Seq-Well')
        tmp$barcode <- gsub('S3_', '', tmp$barcode)
    return(tmp$barcode)
}

get_ding_run <- function(cellmeta, dict, srr) {
    dict <- dict[dict$srr == srr,]
    tmp <- merge(cellmeta, dict, by = 'run')
    ## if (append_srr)
    ##     return(paste(tmp$srr, tmp$barcode, sep = "_"))
    ## else
    ##     return(as.character(tmp$barcode))
    return(unique(paste(tmp$tech, tmp$pbmc, sep = '-')))
}


## get_ding_approved_barcodes(cellmeta = cellmeta, dict = dict, srr = 'SRR9169176')
## get_ding_approved_barcodes(cellmeta = cellmeta, dict = dict, srr = 'SRR9169186')


get_count_table_grouped_by_dings_run <- function(alevin_fns, dict, cellmeta,
                                                 flavour = 'genes_alevin') {

    counts <- list()
    
    ## which runs correspond these to?

    ding <- data.frame(fns = alevin_fns,
                       srr = gsub('/alevin', '', dirname(alevin_fns)),
                       stringsAsFactors = FALSE)

    ## if (params$id == 'CEL-Seq2'){
        ding$run <- sapply(ding$srr, function(x) get_ding_run(cellmeta = cellmeta,
                                                              dict = dict,
                                                              srr = x))
    ## } else {
    ...
    for (run in unique(ding$run)) {

        counts[[run]] <- list()

        for (fn in ding[ding$run == run, 'fns']) {

            srr <- gsub('/alevin','', dirname(fn))

            counts[[run]][[srr]] <- tximeta::tximeta(coldata = data.frame(
                                                         names = flavour,
                                                         files = file.path(params[[flavour]], fn),
                                                         stringsAsFactors = FALSE
                                                     ), type = "alevin")

            counts[[run]][[srr]] <-  as.data.frame(assays(counts[[run]][[srr]])$counts)

            ## get_ding_approved_barcodes(cellmeta = cellmeta,
            ##                            dict = dict,
            ##                            srr = srr)
                                       
            selected <- intersect(get_ding_approved_barcodes(cellmeta = cellmeta,
                                                             dict = dict,
                                                             srr = srr),
                                  colnames(counts[[run]][[srr]]))
            
            counts[[run]][[srr]] <- counts[[run]][[srr]][,selected]
        }
        ## summing up identical barcodes, regardles of the srr (is the run what matters)

        ## rather than merge them, sum them
        genenames <-  rownames(counts[[run]][[srr]])
        ## cbs <- unique(unlist(sapply(counts[[run]], colnames)))
        cbs <- gsub('S3_', '', cellmeta[cellmeta$run == run, 'barcode'])
        tmp  <- data.frame(matrix(0,
                                  nrow = length(genenames),
                                  ncol = length(cbs)),
                           row.names = genenames)
        colnames(tmp) <- cbs
        for (srr in names(counts[[run]])) {
            for (cell in colnames(counts[[run]][[srr]])) {
                tmp[,cell] <- tmp[,cell] + counts[[run]][[srr]][genenames, cell]
            }
        }

        ## remove cells with no data (happens in seqwell)
        idx <- cbs[colSums(tmp, na.rm = TRUE) > 0]
        tmp <- tmp[,idx]
...
genes_fns <- list.files(params$genes_alevin, pattern = 'quants_mat.gz',
                        recursive = TRUE)

counts <- get_count_table_grouped_by_dings_run(alevin_fns = genes_fns,
                                               dict = generate_dict(conf, runmeta),
                                               cellmeta = cellmeta,
                                               flavour = 'genes_alevin') 
...
counts <- get_count_table_grouped_by_dings_run(
    alevin_fns =  repeats_fns,
    dict = generate_dict(conf, runmeta),
    cellmeta = cellmeta,
    flavour = 'repeats_alevin') 
...
common_cells <- intersect(colnames(d$genes_alevin), colnames(d$repeats_alevin))

d$both_alevin <- CreateSeuratObject(
    rbind(as.matrix(GetAssayData(d$genes_alevin, slot = 'counts'))[,common_cells],
          as.matrix(GetAssayData(d$repeats_alevin, slot = 'counts'))[,common_cells]),
    assay = "RNA",
    min.cells = 0,
    min.features = 0,
    names.field = 1,
...
```{r, eval = FALSE}
speed_up_process_featurecounts_grouped_by_cb_umi <- function(fn, id = "",
                                                    annotate_genes = FALSE,
                                                    accepted_barcodes = 'void') {
    ## Featurecounts-based repeats
    counts <- data.table::fread(fn, header = FALSE)
...
for (item in grep('repeats|genes',
    ...
    ding <- data.frame(fns = fns,
                       srr = sapply(strsplit(basename(fns), '[\\._]'), function(x) return(x[1])),
                       stringsAsFactors = FALSE)
    ...
    counts <- list()
    for (run in unique(as.character(ding$run))) {
        message(sprintf('%s %s', item, run))
        counts[[run]] <- list()
        for (fn in ding[ding$run == run, 'fns']) {
            message(fn)
            
            srr <- sapply(strsplit(basename(fn), '[\\._]'), function(x) return(x[1]))
            
            ## print(get_ding_approved_barcodes(cellmeta, dict, srr))
            counts[[run]][[srr]] <- process_featurecounts_grouped_by_cb_umi(
                file.path(params[[item]], fn),
                annotate_genes = FALSE,
                accepted_barcodes = get_ding_approved_barcodes(cellmeta, dict, srr))
        }
        
        ## rather than merge them, sum them
        genenames <-  unique(unlist(lapply(counts[[run]], rownames)))

        cbs <- unique(gsub('[0-9SR_]', '', cellmeta[cellmeta$run == run, 'barcode']))
        ...
        for (srr in names(counts[[run]])) {
            message(srr)
            for (cell in colnames(counts[[run]][[srr]])) {
                tmp[genenames,cell] <- tmp[genenames,cell] + counts[[run]][[srr]][genenames, cell]
            }
        }
        counts[[run]] <- NULL
        ## remove cells with no data (happens in seqwell)
        ...
        ## colnames(counts[[run]]) <- sprintf('%s_%s', run, colnames(counts[[run]]))
    }

    message('looped till here')
    ## check to add undetected features
    genenames <- unique(unlist(sapply(counts, rownames)))
    for (run in names(counts)) {
        missing <- setdiff(genenames, rownames(counts[[run]]))
        zeroes <- as.data.frame(matrix(0, ncol = ncol(counts[[run]]), nrow = length(missing)))
        colnames(zeroes) <- colnames(counts[[run]])
        rownames(zeroes) <- missing
        counts[[run]] <- rbind(counts[[run]], zeroes)
        counts[[run]] <- counts[[run]][genenames,]
...
saveRDS(d, file = paste0(params$seurat_output, '.debug') ) # todo remove
```
...
GetAssayData(d$repeats_star_unique)[1:3, 1:3]
...
```{r debugpreqc}
saveRDS(object =d, file = paste0(params$seurat_output, '.debugpreqc')) # todo remove
```
...
libsize.drop <- isOutlier(d$genes_alevin$nCount_RNA, nmads = 3, type = "both", log = TRUE)
feature.drop <- isOutlier(d$genes_alevin$nFeature_RNA, nmads = 3, type = "both", log = TRUE)
mito.drop <- isOutlier(d$genes_alevin$percent.mt, nmads = 3, type = "higher")

selected_cells <- colnames(d$genes_alevin)[!(libsize.drop | feature.drop | mito.drop)]
...
Filtering out cells according to the genes expression, and forwarding the selection to other layers
