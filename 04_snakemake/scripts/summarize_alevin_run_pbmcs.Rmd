---
title: "Within sample report: PBMC, alevin"
...
params <- list(identifier = "pbmc8k",
               genes_alevin = '/home/imallona/repeats_sc/runs/pbmc8k/alevin/genes/alevin/quants_mat.gz',
               repeats_alevin = '/home/imallona/repeats_sc/runs/pbmc8k/alevin/repeats/alevin/quants_mat.gz',
               regress_genes_nCount = FALSE,
               regress_genes_nFeatures = FALSE,
               seurat_output  = '/home/imallona/tmp/test.RData',
               aris_output  = '/home/imallona/tmp/aris.RData')
...
d$both <- CreateSeuratObject(counts =  rbind(as.matrix(GetAssayData(d$genes_alevin,
                                                          slot = 'counts'))[,cells],
                                            as.matrix(GetAssayData(d$repeats_alevin,
                                                          slot = 'counts'))[,cells]),
                             project = 'both_alevin')

## stopifnot(ncol(d$both) == ncol(d$genes_alevin))
rm(cells)
```
...
d$genes_alevin[["percent.mt"]] <- PercentageFeatureSet(d$genes_alevin, pattern = "-MT-")
...
```{r }
aris_stored <- list()
aris_stored[['genes__repeats_alevin']] <- aris
```

## Both vs genes (alevin)

```{r, results = 'asis', fig.width = 4, fig.height = 4, cache = FALSE}
aris <- list()
i <- 1
for (both_res in seq(0.2, 2, 0.2)) {
    both_res <- sprintf('RNA_snn_res.%s', both_res)
    
    for (genes_res in seq(0.2, 2, 0.2)) {
        genes_res <- sprintf('RNA_snn_res.%s', genes_res)

        cells <- intersect(rownames(d$both@meta.data), rownames(d$genes_alevin@meta.data))
        
   

        aris[[i]] <- c(sprintf('both_%s (n=%s)', both_res,
                               length(unique(d$both@meta.data[cells, both_res]))),
                       sprintf('genes_cr_%s (n=%s)', genes_res,
                               length(unique(d$genes_alevin@meta.data[cells, genes_res]))),
                       adjustedRandIndex(d$both@meta.data[cells,both_res],
                                         d$genes_alevin@meta.data[cells,genes_res]))
        i <- i +1
    }    
}

aris <- do.call(rbind.data.frame, aris)
colnames(aris) <- c('both_res', 'genes_res', 'ari')
aris$ari <- as.numeric(as.character(aris$ari))
...
```{r, fig.width = 6, fig.height = 6}
aris_wide <- reshape2::dcast(aris, both_res~genes_res, value.var="ari")

rownames(aris_wide)<- aris_wide$both_res
aris_wide <- aris_wide[,-1]
...
```{r store_aris}
aris_stored <- list()
aris_stored[['both__genes_alevin']] <- aris
```
