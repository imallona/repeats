...
```{r, eval = FALSE}
## setClass("Snakemake",
##          representation(input = 'list'))

## snakemake <- new('Snakemake', input = list(genes_cellranger = "/home/imallona/repeats_sc/runs/frozen_pbmc_donor_a/cellranger_standard/outs/filtered_feature_bc_matrix/barcodes.tsv.gz",
...
colnames(tmp) <- gsub(".*([A-Z]{14})-1.bam","\\1", colnames(tmp))
## tmp <- tmp[,7:ncol(tmp)]
...
```{r}
hist(unlist(ccell), main = "correlation across cells")
...
# Add other layers or repeats quantification
