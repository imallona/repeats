---
title: "Within sample report: PBMC, celseq2"
...
params <- list(identifier = "",
               repeats_bowtie = '/home/imallona/repeats_sc/runs/ding_celseq2/bowtie_repeatome/SRR9169177.bam.featureCounts.gz',
               genes_star = '/home/imallona/repeats_sc/runs/ding_celseq2/star_transcriptome/genes/SRR9169177.bam.featureCounts.gz',
               repeats_star = '/home/imallona/repeats_sc/runs/ding_celseq2/star_transcriptome/repeats/SRR9169177.bam.featureCounts.gz',
               repeats_no_overlap_star = '/home/imallona/repeats_sc/runs/ding_celseq2/star_transcriptome/SRR9169177.bam.featureCounts.gz',
               seurat_output = "/home/imallona/tmp/test.rds",
               repeats_alevin1 = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/repeats/male/alevin/quants_mat.gz',
               genes_alevin1 = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/genes/male/alevin/quants_mat.gz',
               repeats_alevin2 = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/repeats/female/alevin/quants_mat.gz',
               genes_alevin2 = '/home/imallona/repeats_sc/runs/ding_celseq2/alevin/genes/female/alevin/quants_mat.gz',
...
## tmp <- data.table::fread(params$repeats_bowtie, sep = '\t', header = FALSE, fill = TRUE)
...
Alevin reading. They're 6-nt long cellbarcodes, is this right? Yeah, https://bitbucket.org/jerry00/scumi-dev/src/master/lib/scumi/config.yaml says
...
```{r}
genes_alevin1 <- tximeta::tximeta(coldata = data.frame(
                                       names = "genes",
                                       files = params$genes_alevin1,
                                       stringsAsFactors = FALSE
                                   ), type = "alevin")

counts1 <-  assays(genes_alevin1)$counts

genes_alevin2 <- tximeta::tximeta(coldata = data.frame(
                                       names = "genes",
                                       files = params$genes_alevin2,
                                       stringsAsFactors = FALSE
                                   ), type = "alevin")

counts2 <-  assays(genes_alevin2)$counts
...
colnames(counts1) <- paste0('male_', colnames(counts1))
colnames(counts2) <- paste0('female_', colnames(counts2))

counts1$gene <- rownames(counts1)
counts2$gene <- rownames(counts2)
counts <- merge(counts1, counts2, by = 'gene', all = TRUE)
colnames(counts)
rownames(counts) <- counts$gene

counts <- counts[,setdiff(colnames(counts), 'gene')]
...
reps1 <- tximeta::tximeta(coldata = data.frame(
                                       names = "repeats",
                                       files = params$repeats_alevin1,
                                       stringsAsFactors = FALSE
                                    ), type = "alevin")

reps1 <- as.data.frame(assays(reps1)$counts)

reps2 <- tximeta::tximeta(coldata = data.frame(
                                        
                                       names = "repeats",
                                       files = params$repeats_alevin2,
                                       stringsAsFactors = FALSE
                                    ), type = "alevin")

reps2 <- as.data.frame(assays(reps2)$counts)

colnames(reps1) <- paste0('male_', colnames(reps1))
colnames(reps2) <- paste0('female_', colnames(reps2))


reps1$gene <- rownames(reps1)
reps2$gene <- rownames(reps2)

class(reps1)

counts <- merge(reps1, reps2, by = 'gene', all = TRUE)
rownames(counts) <- counts$gene
counts <- counts[,setdiff(colnames(counts), 'gene')]
...
for (res in grep('RNA_snn', colnames(d$genes_alevin@meta.data), value = TRUE)) {
    cat('### ', res, '\n\n')

    tmp <- as.data.frame.matrix(table(d$genes_alevin@meta.data[,res],
                                      d$genes_alevin@meta.data$scina))
...
item <- 'genes_alevin'
print(DimPlot(d[[item]], reduction = "umap", group.by = 'scina', 
              cols = setNames(pal$color, pal$identity), split.by = 'scina'))
...
for (genes_res in seq(0.2, 2, 0.2)) {
    ...
    for (repeats_res in seq(0.2, 2, 0.2)) {
        ...
        cells <- intersect(rownames(d$genes_alevin@meta.data), rownames(d$repeats_alevin@meta.data))
        
   

        aris[[i]] <- c(sprintf('genes_%s (n=%s)', genes_res,
                               length(unique(d$genes_alevin@meta.data[cells, genes_res]))),
                       sprintf('repeats_cr_%s (n=%s)', repeats_res,
                               length(unique(d$repeats_alevin@meta.data[cells, repeats_res]))),
                       adjustedRandIndex(d$genes_alevin@meta.data[cells,genes_res],
                                         d$repeats_alevin@meta.data[cells,repeats_res]))
        i <- i +1
    }    
}
...
for (genes_res in seq(0.2, 2, 0.2)) {
    genes_res <- sprintf('RNA_snn_res.%s', genes_res)
    rownames(d$genes_alevin@meta.data) <- d$genes_alevin@meta.data$cell
    known <- rownames(d$genes_alevin@meta.data[d$genes_alevin@meta.data$scina != 'unknown',])

    aris[[i]] <- c(sprintf('genes_%s (n=%s)', genes_res,
                           length(unique(d$genes_alevin@meta.data[known, genes_res]))),
                   sprintf('scina_%s (n=%s)', 'scina',
                           length(unique(d$genes_alevin@meta.data[known, 'scina']))),
                   adjustedRandIndex(d$genes_alevin@meta.data[known, genes_res],
                                     d$genes_alevin@meta.data[known, 'scina']))
