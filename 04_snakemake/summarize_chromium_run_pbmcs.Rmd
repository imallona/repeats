...
params <- list(identifier = "frozen_pbmc_donor_a",
               ...
               genes_alevin = '/home/imallona/repeats_sc/runs/frozen_pbmc_donor_a/alevin/genes/alevin/quants_mat.gz',
               repeats_alevin = '/home/imallona/repeats_sc/runs/frozen_pbmc_donor_a/alevin/repeats/alevin/quants_mat.gz')
...
d$both_alevin <- CreateSeuratObject(counts =  rbind(as.matrix(GetAssayData(d$genes_alevin,
                                                          slot = 'counts'))[,cells],
...
Compare per-cell concatenated counts.


```{r}


repeats_shared <- Reduce(intersect, sapply(d[grep('repeats', names(d))], rownames))
## cells_shared <- Reduce(intersect, sapply(d[grep('repeats', names(d))], colnames))
...
Similarly, for Spearman's correlations (it's angular distances! directly the rho value as distance).
...
head(oned_cor)
hc_cor <- hclust(d = as.dist(oned_cor))
