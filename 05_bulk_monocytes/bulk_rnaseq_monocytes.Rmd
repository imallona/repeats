...
params <- list(identifier = "bulk_GSE80095", 
          ...
          repeats_only_unique = "/home/imallona/repeats_sc/runs/bulk_GSE80095/star_transcriptome/repeats_no_overlap/unique_reads/bulk_GSE80095_repeats_not_overlapping_genes.counts.gz",
          repeats_only_multi = "/home/imallona/repeats_sc/runs/bulk_GSE80095/star_transcriptome/repeats_no_overlap/multimappers/bulk_GSE80095_repeats_not_overlapping_genes.counts.gz",
...
```{r}
annot <- read.table('/home/imallona/repeats_sc/data/bulk_GSE80095//monocytes.conf',
                    header = FALSE, sep = ',')

colnames(annot)[1] <- c('srr')
colnames(annot)[9] <- 'sample'
...
for (item in setdiff(names(params), c('identifier'))){
...
```{r logcpm}
d <- lapply(d, function(x) edgeR::cpm(x, log = TRUE))
```

# remove nonexpressed genes and re-normalize
...
Using the gender? or rather add another dataset (not monocytes), like T cells, here
