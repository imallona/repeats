...
Get rid of anything not a Monocyte or T-cell

```{r}
annot <- annot[annot$sample %in% c('Monocytes', 'T_cells'),]
...
for (item in setdiff(names(params), c('identifier', 'de_output', 'config_fn', 'expression_output'))){

    d[[item]] <- read_featurecounts(
        params[[item]],
        identifier = sprintf('%s_%s', params$identifier, params[[item]]),
        add_gene_symbols = grepl('genes', item))[,annot$srr]
...
Monocytes vs T cells

```{r edgeR-define-contrasts}
(conts <- as.data.frame(
   makeContrasts(
       contrasts = 'sampleMonocytes-sampleT_cells',
       levels = des)))
