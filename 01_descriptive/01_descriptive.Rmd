...
```{r}
new.cluster.ids <- c("Naive CD4 T", "Memory CD4 T + CD8", "T CD8", "NK", "B", "DC", 
    "B", "T CD8", "Platelet")
...
reps <- FindClusters(reps, resolution = 0.5)
```
...
target <- pbmc@meta.data[,c('RNA_snn_res.0.5', reps_markers_char)]

foo <- melt(target, id.vars='RNA_snn_res.0.5')
...
for (feature in reps_markers_char) {
    ...
    plot(y = pbmc@meta.data[,feature],
         ...
         xlab = 'number of features (genes)',
         ylab = 'number of repeats [log10(x+1)]',
         col = ac(as.numeric(pbmc@meta.data$RNA_snn_res.0.5), 0.5))
    ...
    plot.new()
    legend('topleft',
           legend= levels(pbmc@meta.data$RNA_snn_res.0.5),
           pch = 19,
           col = 1: max(as.numeric(pbmc@meta.data$RNA_snn_res.0.5)),
...
target <- pbmc@meta.data[,c('RNA_snn_res.0.5', selected)]
...
```{r}
foo <- data.frame(genes = pbmc@meta.data[,'RNA_snn_res.0.5'],
                 cell = row.names(pbmc@meta.data))

bar <- data.frame(repeats = reps@meta.data[,'RNA_snn_res.0.5'],
...
target <- FetchData(pbmc, cell_type_features)
