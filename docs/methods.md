...
For sc Chromium specifically, the alignment+counting architecture is configurable via the top-level config flag `sc_count_mode`. The two modes differ in how the per-feature_set count matrices are produced from the same fastqs.
...
`sc_count_mode: gene_align_recount`. One STARsolo run per library, aligned with `genes.gtf` for splice junctions and gene counting. The resulting BAM (with `CB` and `UB` tags) is then re-counted by `workflow/scripts/sc_count_features.py` against each repeat feature_set GTF. This avoids three independent STAR alignments per library and ensures splice junctions are inferred from the gene model only — not from repeat GTFs which would otherwise inject spurious junctions. Outputs land at `<base>/starsolo_tagcount/<sample>/<mode>_<feature_set>/` so they coexist on disk with `per_featureset` results from sibling yamls and don't trigger downstream rebuilds.

The recount script `sc_count_features.py` is the load-bearing piece in `gene_align_recount` mode. Its design choices, exposed as CLI flags and configurable from the workflow config:
...
- UMI deduplication (`--umi-dedup`). `1mm_all` is the default and matches STARsolo's `--soloUMIdedup 1MM_All`: per `(CB, gene_id)`, UMIs within Hamming-1 of each other collapse via union-find before counting. The alternative `exact` setting counts distinct UB strings without collapsing and is for debugging only — it systematically over-counts unique molecules by ~2-5% because sequencing errors create spurious UMIs.
...
Correctness is exercised by `test/unit/test_sc_count_features.py` against synthetic GTFs and BAMs covering exonic vs intronic counting, multi-feature ambiguity, missing CB/UB tags, off-whitelist barcodes, multimapper handling, UMI dedup at exact and Hamming-1 distance, and family/class rollups. In unique mode with `1mm_all` dedup, the recount is expected to agree exactly with STARsolo's gene matrix produced from the same BAM; the comparison Rmd asserts this as a regression check before the project relies on the recount in production.
