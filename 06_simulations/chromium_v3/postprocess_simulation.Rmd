...
for (cell in unique(truth$cell)) {
    ...
    if (cell %in% d$shao$RG) {
        true <- truth[truth$cell == cell, 'id']

        observed <- d$shao$merged[d$shao$merged$RG == cell,]        
        ...
        multi <- observed[duplicated(observed$sim_locus), 'sim_locus']

        wrong <- unique(observed[!observed$overlaps, 'sim_locus'])
