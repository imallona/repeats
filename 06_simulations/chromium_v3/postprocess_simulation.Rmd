...
output: 
  html_document:
    ...
    repeats_unique: ""
    repeats_multi: ""
    repeats_lenient_unique: ""
    repeats_strict_unique: ""
    repeats_lenient_multi: ""
    repeats_strict_multi: ""
...
flavour_annot <- read.csv(text="id,multimapping,overlap,gtf
truth,truth,truth,truth
repeats_alevin,multimapping,all,genes_decoy
repeats_alevin_no_decoy,multimapping,all,repeats
repeats_lenient_unique,unique,all,genes
repeats_strict_unique,unique,strict,genes
repeats_lenient_multi,multimapping,all,genes
repeats_strict_multi,multimapping,strict,genes
repeats_bowtie_unique,unique,all,repeats
scte,multimapping,all,repeats
shao,multimapping,all,repeats
repeats_unique,unique,all,repeats
repeats_multi,multimapping,all,repeats", header = TRUE)
```
...
params <- list(repeats_alevin_locus = "/home/imallona/repeats_sc/runs/sim_5k_pbmc_v3/alevin/repeats_per_locus/alevin/quants_mat.gz",
               ...
               repeats_unique = "/home/imallona/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_repeats/unique_reads/sim_5k_pbmc_v3_repeats.counts.gz",
               repeats_multi = "/home/imallona/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_repeats/multimappers/sim_5k_pbmc_v3_repeats.counts.gz",
               repeats_lenient_unique = "/home/imallona/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_standard/unique_reads/sim_5k_pbmc_v3_repeats.counts.gz",
               repeats_strict_unique = '~/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_standard_not_overlapping_genes/unique_reads/sim_5k_pbmc_v3_repeats_not_overlapping_genes.counts.gz',
               repeats_lenient_multi = "/home/imallona/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_standard/multimappers/sim_5k_pbmc_v3_repeats.counts.gz",
               repeats_strict_multi = '~/repeats_sc/runs/sim_5k_pbmc_v3/count_repeats_loci_on_cellranger_standard_not_overlapping_genes/multimappers/sim_5k_pbmc_v3_repeats_not_overlapping_genes.counts.gz',
...
featurecounts_runs <- c('repeats_unique',
                        'repeats_multi',
                        'repeats_lenient_unique',
                        'repeats_strict_unique',
                        'repeats_lenient_multi',
...
Then, merge with the SAM file to assign TE_x identifers to CBs, but also to the original simulated read name/origin. Merging is based on feature and RG identity.

```{r}
d$shao$merged <- merge(d$shao$merged, d$shao$sam, by.x = c('feature', 'RG'),
                       by.y = c('V4', 'V2'),
                       all.x = TRUE)
...
```{r}
fd2 <- as.data.frame(dplyr::group_by(fd, flavour, variable) %>%
              dplyr::summarize(mean = mean(value),
                        sd = sd(value),
                        lower = mean(value) - sd(value),
                        upper = mean(value) + sd(value)) %>%
              dplyr::arrange(flavour) %>%
              dplyr::group_by(flavour)) 
...
ggplot(fd2, aes(x=flavour, weight=mean, ymin=lower, ymax=upper, fill=variable)) +
    ...
    scale_fill_manual("mapping\ncategory", values = pal_mapping[levels(fd2$variable)]) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    xlab('strategy') +
    ylab('repeats loci per cell (counts)')
```


Alternative with just the correct/wrong/missing

```{r}
curr <- fd2[fd2$variable != 'multimapping',]
ggplot(curr,
       aes(x=flavour, weight=mean, ymin=lower, ymax=upper, fill=variable)) +
    geom_bar      (position=position_dodge(), aes(y=mean), stat="identity") +
    geom_errorbar (position=position_dodge(width=0.9), colour="black", width = 0.2) +
    theme_bw() +
    scale_x_discrete(limits = unique(fd2$flavour)) +
    ## scale_fill_grey(name = 'mapping\ncategory') +
    scale_fill_manual("mapping\ncategory",
                      values = pal_mapping[levels(curr$variable)]) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    xlab('strategy') +
    ylab('repeats loci per cell (counts)')
```
...
ggplot(fd2, aes(x=flavour, weight=mean, ymin=lower, ymax=upper, fill=variable)) +
    ...
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    xlab('strategy') +
    ylab('repeats loci per cell (proportion)')
```
...
## grouped$variable <- factor(grouped$variable, levels = c('correct',
##                                                         'multimapping', 'missing', 'wrong'))
...
grouped %>% ggplot(aes(x=Group.1, y=value, fill =variable)) +
    ...
    theme_bw() +
    scale_x_discrete(limits = unique(fd$flavour)) +
    scale_fill_manual("mapping\ncategory", values = pal_mapping[levels(grouped$variable)]) + 
    ## scale_fill_grey(name = 'mapping\ncategory') +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    xlab('strategy') +
...
## percentages
tmp <- plyr::ddply(tmp, .(flavour), transform, percent = counts/sum(counts) * 100)
tmp$percent[tmp$percent == NA] <- 0
tmp$label <- sprintf("%.1f", tmp$percent)

## to hide labels for very small bars
tmp$label <- ifelse(as.numeric(tmp$label) >= 0.2, tmp$label, NA)
```
...
Better facet wrap labels


```{r}
tmp$mapping <- ifelse(tmp$gtf == 'repeats', 'repeats', 'genes')
tmp$mapping[tmp$gtf == 'truth'] <- 'truth'
tmp$mapping[tmp$gtf == 'genes_decoy'] <- 'decoy'

tmp$multi <- ifelse(tmp$multimapping == 'multimapping', 'yes', 'no')
tmp$multi[tmp$multimapping == 'truth'] <- ''

tmp$repeats <- ifelse(tmp$overlap == 'strict', 'strict', 'lenient')
tmp$repeats[tmp$overlap == 'truth'] <- ''
...
ggplot(tmp, aes(fill = class, y = counts, x = flavour)) + 
    ...
    scale_fill_manual(values = class_pal) +
    geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 2) +
    facet_grid(~mapping*multi*repeats,
               labeller = label_context, scales = "free", space = "free") +
    ylab('# repeats loci')
```

```{r}
# the empty one is the truth
tmp$mapping <- factor(tmp$mapping, levels = c('truth', 'genes', 'repeats', 'decoy'))
tmp$multi <- factor(tmp$multi, levels = c('', 'yes', 'no'))
tmp$repeats <- factor(tmp$repeats, levels = c('', 'lenient', 'strict'))
```

```{r final_class, fig.width=7, fig.height=5}
ggplot(tmp, aes(fill = class, y = counts, x = flavour)) + 
    geom_bar(position="stack", stat="identity", width = 1, colour = 'black')  +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    scale_fill_manual(values = class_pal) +
    geom_text(aes(label = label), position = position_stack(vjust = 0.5), size = 2) +
    facet_grid(~repeats*mapping*multi,
                scales = "free", space = "free") +
    ylab('# repeats loci') +
    guides(shape = guide_legend(override.aes = list(size = 3)),
           fill=guide_legend(ncol=2))

grid.text("repeat definition", x = unit(0.78, "npc"), y = unit(0.97, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
grid.text("mapping", x = unit(0.78, "npc"), y = unit(0.935, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
grid.text("multimapping", x = unit(0.78, "npc"), y = unit(0.90, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
```
...
<!-- ```{r} -->
<!-- p <- ggplot(data = tmp, -->
<!--             mapping = aes_string(x = "class",  -->
<!--                                  y = "flavour")) + -->
<!--     geom_point(mapping = aes_string(size = "counts", -->
<!--                                     color = "percent")) + -->
<!--     scale_color_viridis() + -->
<!--     scale_x_discrete(limits = rev(levels(tmp$class))) + -->
<!--     scale_y_discrete(limits = rev(levels(tmp$flavour))) + -->
    
<!--     ## scale_size_identity(trans="log",guide="legend") + -->
<!--     theme(panel.background = element_blank(), -->
<!--           axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+ -->
<!--     facet_grid(multimapping*overlap~., scales = "free", space = "free", drop = TRUE) -->
...
Multinomial exact goodness of fit tests, each flavour vs the truth

```
Error in xmulti(expected, observed, detail = 3, histobins = TRUE) : 
...
tests <- list()
outer_tests <- list()
for (flavour in setdiff(levels(tmp$flavour), 'truth')) {
    ## print(flavour)

    ##     expr: vector containing expectation. The length should be the same
    ##       as that of 'obs' and they should be non-negative summing to
    ## '> 0'. They need not be integers or sum to one.
    
    expected <- tmp[tmp$flavour == 'truth', 'percent']/100
    names(expected) <- tmp[tmp$flavour == 'truth', 'class']
    
    ## obs: vector containing the observed numbers. All are non-negative
    ##          integers summing to '> 0'.
    observed <- tmp[tmp$flavour == flavour, 'counts']
    names(observed) <- tmp[tmp$flavour == flavour, 'class']

    observed <- observed[names(expected)]
    stopifnot(names(observed) == names(expected))

    ## 'observed' shoud be non-negative integers
    observed[observed==0] <- 1
    
    tests[[flavour]] <- xmonte(expr = expected, obs = round(observed), detail = 1)
    ## print(tests[[flavour]])
}

sapply(tests, function(x) return(x$asymptotic.p.value))
outer_tests[['class']] <- tests
```

<!-- Trying with another multinomial goodness of fit test -->

<!-- ```{r} -->

<!-- ExactMultinom:: multinom.test( -->
<!--                     x = round(observed), -->
<!--                     p = expected, -->
<!--                     stat = "Prob", -->
<!--                     method = "asymptotic", -->
...
```{r final_family, fig.width=7, fig.height=5}
tmp$mapping <- ifelse(tmp$gtf == 'repeats', 'repeats', 'genes')
tmp$mapping[tmp$gtf == 'truth'] <- 'truth'
tmp$mapping[tmp$gtf == 'genes_decoy'] <- 'decoy'
...
for (flavour in setdiff(levels(tmp$flavour), 'truth')) {
    ...
    expected <- tmp[tmp$flavour == 'truth', 'percent']/100
    names(expected) <- tmp[tmp$flavour == 'truth', 'family']
    
    ## obs: vector containing the observed numbers. All are non-negative
    ##          integers summing to '> 0'.
    observed <- tmp[tmp$flavour == flavour, 'counts']
    names(observed) <- tmp[tmp$flavour == flavour, 'family']
...
```{r final_name, fig.width=7, fig.height=5}
tmp$mapping <- ifelse(tmp$gtf == 'repeats', 'repeats', 'genes')
tmp$mapping[tmp$gtf == 'truth'] <- 'truth'
tmp$mapping[tmp$gtf == 'genes_decoy'] <- 'decoy'
...
grid.text("repeat definition", x = unit(0.85, "npc"), y = unit(0.97, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
grid.text("mapping", x = unit(0.85, "npc"), y = unit(0.935, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
grid.text("multimapping", x = unit(0.85, "npc"), y = unit(0.90, "npc"),
          gp = gpar(fontsize = 10), just = 'left')
```
